...
async function readInstalledShopProducts(shop: string, token: string, limit: number) {
  const response = await fetch(
    `https://${shop}/admin/api/2026-07/graphql.json`,
    {
      ...
      body: JSON.stringify({
        query: `
          query InstalledShopProducts($first: Int!) {
  ...
  if (!response.ok || payload.errors || !payload.data) {
    throw new Error("Unable to read installed Shopify store products");
  }
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const products = await readInstalledShopProducts(shop, token, limit);
    ...
    await sql`
      ...
      VALUES (
        ...
        'SHOPIFY_INSTALLED_PRODUCTS_READ',
        'SHOPIFY_STORE',
        ${shop},
        'SUCCESS',
        ${JSON.stringify({
    ...
    return Response.json(
      {
        ...
        message:
          error instanceof Error
            ? error.message
            : "Unable to run installed Shopify read test",
      },
      { status: 500 }
    );
  }
