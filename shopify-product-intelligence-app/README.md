...
- `GET /api/shopify/installed-orders?shop=stonewick-store.myshopify.com&limit=10`
...
- First-test scopes: `read_products`, `read_orders`, `write_products`
...
Run the harmless Stonewick order read test after the app is reapproved for
`read_orders`:

```powershell
$secretLine = Get-Content .env.local | Where-Object { $_ -like "CRON_SECRET=*" } | Select-Object -First 1
$cronSecret = $secretLine.Substring($secretLine.IndexOf("=") + 1).Trim('"')
Invoke-RestMethod -Method GET -Uri "http://127.0.0.1:3000/api/shopify/installed-orders?shop=stonewick-store.myshopify.com&limit=10" -Headers @{ Authorization = "Bearer $cronSecret" }
```

This test only reads order data from the installed shop. It does not publish,
change prices, change inventory, create purchases, activate ads, or make any
other live write.
